-- ============================================================
-- SALES BREAKDOWN BY INTERVAL, LOCATION, CATEGORY
-- WITH LOCATION FILTER, CATEGORY FILTER, MIN CATEGORY PERIOD VALUE,
-- AND MIN INTERVAL SALES COUNT/VALUE (HAVING)
--
-- PERCENTAGES ARE CALCULATED AGAINST ALL SALES (INCLUDING UNFILTERED CATEGORIES)
-- EVEN IF YOU FILTER TO ONLY ONE CATEGORY, ITS SHARE WILL BE BASED ON TOTAL SALES.
--
-- Modify the parameters below:
--   start_date                : first day of the period
--   end_date                  : last day of the period
--   interval_unit             : 'day', 'week', 'month', 'quarter', 'year'
--   selected_locations        : array of location names – use ARRAY[]::text[] for all
--   selected_categories       : array of category names – use ARRAY[]::text[] for all
--   min_category_value        : exclude categories with total period sales < this
--   min_interval_sales_count  : exclude groups with fewer than this many sales in the interval
--   min_interval_sales_value  : exclude groups with total sales value less than this in the interval
-- ============================================================
...
WITH params AS (
    SELECT
        ...
        1::integer                     AS min_interval_sales_count,   -- at least 5 sales in the interval
        100::numeric                   AS min_interval_sales_value     -- at least 100 euros in the interval
),
...
aggregated_filtered AS (
    ...
    HAVING
        COUNT(*) >= (SELECT min_interval_sales_count FROM params)        -- at least N sales
        AND SUM(total_price) >= (SELECT min_interval_sales_value FROM params)   -- at least €X
),
...
final_data AS (
    SELECT
        ...
        -- Within‑location percentages (vs. that location's total)
        ...
        -- Grand‑total percentages (vs. all locations and categories)
    ...
    CROSS JOIN params p
    WHERE cpt.total_category_value >= p.min_category_value   -- exclude low‑total categories over whole period
)
